...
func testPortsExists(t *testing.T) {
	...
	e, err := PortExists(ctx, tx, o.UUID)
	if err != nil {
...
func testPortsFind(t *testing.T) {
	...
	portFound, err := FindPort(ctx, tx, o.UUID)
	if err != nil {
		t.Error(err)
	}
...
var (
	portDBTypes = map[string]string{`UUID`: `uuid`, `Key`: `text`, `Name`: `text`, `City`: `text`, `Country`: `text`, `Province`: `text`, `Timezone`: `text`, `Code`: `text`, `Alias`: `ARRAYcharacter varying`, `Regions`: `ARRAYcharacter varying`, `Unlocs`: `ARRAYcharacter varying`, `Xcoord`: `text`, `Ycoord`: `text`}
	_           = bytes.MinRead
)
...
func testPortsUpsert(t *testing.T) {
	t.Parallel()
	...
	if err = o.Upsert(ctx, tx, false, nil, boil.Infer(), boil.Infer()); err != nil {
		t.Errorf("Unable to upsert Port: %s", err)
	}
	...
	if err = randomize.Struct(seed, &o, portDBTypes, false, portPrimaryKeyColumns...); err != nil {
		t.Errorf("Unable to randomize Port struct: %s", err)
	}
